...
#           : a flag to activate a tip or '' to deactivate it
...
# LastUpdate: 2022.05.11Z06:39:32 *
#*******************************************************************************
#
### INISection ##
...
#       echo "unzip -p ${zipFName} ${crxFName} | crx2rnx > ${rnxFName}"
      ...
      for hh in ${hours}; do
        for mm in ${minutes}; do
          echo -ne $(teqc +qc -plot -report -R -E -C -S -O.int 15 -st ${hh}${mm}00 \
               -e ${hh}$((${mm}+14))45 ${rnxFName} 2> /dev/null | grep -e \
               '\(^Epochs w/ observations  :\|Poss. . of obs epochs   :\)' | cut \
               -d':' -f2)| awk '{total=100*$2/$1;printf("%s",(total>=100)?"#":"-")}'
        done
        echo -ne " "
      done
      rm ${rnxFName}
...
31008e61be3ec8d006d8516ec78412e3587b5008c95223fd6ccd7ff4675aab26  -
Hash replaced automatically by sed at 2022.05.11Z06:39:32
#
