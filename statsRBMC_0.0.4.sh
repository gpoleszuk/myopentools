...
# Version   : 0.0.4 - Using colors on labels
#           : 0.0.3 - Check dependencies before to start. Abort case not found
#           : 0.0.2 - Auto update the "^# LastUpdate: " field
#           : 0.0.1 - Initial idea and code
# LastUpdate: 2022.05.11Z05:52:26 *
#*******************************************************************************
#
...
lbl_OK_="[\033[1;32m.OK.\033[0m]"
lblErro="[\033[1;31mERRO\033[0m]"
lblWarn="[\033[1;34mWARN\033[0m]"
lblStop="[\033[1;35mSTOP\033[0m]"
...
#>&2   echo -e "[\033[1;32m.OK.\033[0m] '${dependency}'  found at   ${result}"
    else
#>&2 echo -e "[\033[1;31mERRO\033[0m] ${dependency} dependency not found"
>&2 echo -e "${lblErro} ${dependency} dependency not found"
      exit 1
    fi
  done
#>&2 echo -e "[\033[1;32m.OK.\033[0m] ${dependenciesFound}"
...
#>&2    echo -ne "[${progressChars[${i}]}]";
  echo "[${progressChars[${i}]}]";
#  return "[${progressChars[${i}]}]";
}
#------------------------------------------------------------------------------
...
#>&2     echo -ne "[${progressChars[${i}]}] ${iteration}\\x0d";
    sleep .1;
  done;  echo ""
}
#
...
>&2     echo "[INFO] ${commandFile} does not exist. Creating it..."
    ...
    for doy in $(seq -f"%03.0f" ${minDoyToStart} 1 ${maxDoyPerYear}); do
      ...
      then
        ...
        echo "!prn2err.sh '[INFO] Sleeping for 10 seconds...\\x0a'" >> ${commandFile}
...
# ---------------------------------------------------------------------
  ...
  for doy in $(seq -f"%03.0f" ${minDoyToStart} 1 ${maxDoyPerYear}); do
    if [ ! -e ${rbmcFileList} ]; then
>&2       echo -ne "${lblErro} ${rbmcFileList} disappeared!!! Aborting...."; exit 1; fi;
...
cfd2022203f8261d9b1efbc8edc50c18d194bb4e666b5f38c30cc00a5cced967  -
